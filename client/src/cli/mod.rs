...
#[component]
pub(super) fn CliDetails(manifest: ToolManifest, on_back: Callback<()>) -> Element {
    ...
    rsx! {
        ...
        header { class: "extension-browser__heading", Terminal {} div {
            h1 { "{manifest.title}" }
            p { "CLI · {manifest.version} · {manifest.license}" }
            p { "{manifest.summary}" }
        } }
        ...
        div { role: "tabpanel",
            if let Some(plan) = plan {
                Markdown { link_base: manifest.homepage.clone(), image_base: manifest.homepage.clone(), source: format!("## 首次连接此电脑\n\n在终端执行一次，注册本机助手；之后可直接点击上方安装。需要 Node.js 和 npm。\n\n```sh\nnpx -y @zjarlin/aio helper install\n```\n\n## 安装与卸载\n\n也可以在终端执行：\n\n```sh\n{install}\n```\n\n恢复配置并卸载：\n\n```sh\n{uninstall}\n```") }
            ...
            a { href: manifest.homepage, target: "_blank", rel: "noopener noreferrer", "项目主页 ↗" }
        }
    }
}
